##Pruebas de hip贸tesis para los valores de las 5 estaciones en el mes de 
...
#Se realiza la prueba de hip贸tesis en cada zona, considerando la varianza distinta
#S贸lo se toman en cuenta las que superaron la prueba de normalidad en ambos a帽os
Zonas_PH<-c("NE", "NO","CE")

Hipotesis<-c() #Se acepta la hip贸tesis alternativa 2020<2019? True se acepta False se rechaza
for(i in Zonas_PH){
...
#En todas las zonas estudiadas se muestra una diferencia significativa entre a駉s para este mes
...
#solamente se utilizan las zonas que mostraron normalidad en ambos a帽os
Zonas_PH25<-c("CE","NO","NE","SE")
#Se agrupan los datos por zona y se determinan los valores para las pruebas

Hipotesis<-c() #Se rechaza la hip贸tesis nula 2020<2019? True se rechaza False se acepta de que
for(i in Zonas_PH25){
